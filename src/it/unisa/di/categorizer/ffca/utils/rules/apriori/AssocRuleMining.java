...
public class AssocRuleMining extends JFrame {
	...
	public void readInputDataSet() throws IOException {
		...
		while (line != null) {
			nline++;
			System.out.println("Read line: " + nline);
			StringTokenizer dataLine = new StringTokenizer(line);
			int numberOfTokens = dataLine.countTokens();
			if (numberOfTokens == 0)
				break;
	...
	protected String outputItemSet(short[] itemSet) throws IOException {
	...
	protected String getDuration(double time1, double time2) {
		double duration = (time2 - time1) / 1000;
		return ("Generation time = " + twoDecPlaces(duration) + " seconds ("
				+ twoDecPlaces(duration / 60) + " mins)");
	}
	...
	public static void main(String[] args) {
		new AssocRuleMining().setVisible(true);
	}
}
